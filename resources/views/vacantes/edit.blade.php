...
@section('content')
    <h2 class="text-2xl text-center mt-10">Editar vacante {{$vacante->titulo}}</h2>
    <form action="{{route("vacantes.update",["vacante"=>$vacante->id])}}" method="POST" class="max-w-lg mx-auto my-10">
        @csrf
        @method("PUT")
        <div class="mb-5">
            <label for="titulo" class="block text-gray-700 text-sm mb-2">Titulo vacante</label>
            <input id="titulo" type="text" class="p-3 bg-white rounded form-input w-full @error('password') is-invalid @enderror" placeholder="Titulo de la vacante" name="titulo" value="{{$vacante->titulo}}">
            @error('titulo')
